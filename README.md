...
CI runs the unit test suite (`python -m unittest discover`), then builds the Docker image and polls `/health` until the container reports ready, on every pull request and push to `main`; it does not run the golden QA eval, which stays a deliberate manual step (non-deterministic, calls a paid LLM API). Every accepted `main` revision also publishes a versioned image to `ghcr.io/juan-casimiro/ai-research-assistant` (tagged by commit SHA, plus `latest`) — delivery stops at that artifact, with no automatic deployment anywhere (see [ADR-003](./adr/003-deployment-and-containerisation.md)).
