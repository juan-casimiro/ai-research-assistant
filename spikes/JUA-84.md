...
This report records an unmerged experiment with SmolLM2-1.7B-Instruct.
The default branch does not implement `LLM_PROVIDER=ollama`. Experimental code,
dependencies, and tests are preserved on
[`cas-agent/JUA-84-local-ollama-spike`](https://github.com/juan-casimiro/ai-research-assistant/tree/cas-agent/JUA-84-local-ollama-spike),
at commit [`b395398`](https://github.com/juan-casimiro/ai-research-assistant/commit/b395398).
Keep that branch as the experiment archive, not a production merge candidate.
Production adapters and model adoption require separate follow-up work.

## Run in Docker

Use a separate checkout of the preserved experiment, so these commands do not
switch your active development branch. From an existing clone:

```bash
git fetch origin cas-agent/JUA-84-local-ollama-spike
git worktree add --detach ../ai-research-assistant-jua84-repro b395398
cd ../ai-research-assistant-jua84-repro
```

Run from that checkout's root. These names, ports, and volumes are separate from
the normal demo stack. No `.env` or Anthropic credentials are passed to the RAG
container. A new Chroma volume is seeded with the bundled demo corpus; an
existing `jua84-chroma` volume is reused. If the spike containers already exist,
use them or follow the cleanup commands below before recreating them. Never
delete an existing corpus volume merely to repeat this experiment.
...
The warm measurement repeated the same question and context, so it may benefit
from prompt-cache reuse as well as an already loaded model. It does not isolate
model-loading overhead or predict latency for a new question. Startup preloading
and a configurable keep-alive period remain untested follow-up ideas.
