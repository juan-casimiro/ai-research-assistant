...
class GroundedAnswer(BaseModel):
    ...
    context_sufficient: bool = Field(
        description=(
            "True only if the provided context explicitly contains the information "
            "requested by the question. False if the requested fact, number, "
            "recommendation, dose, treatment adjustment, threshold, or other specific "
            "detail is not stated, even when the context is relevant and supports "
            "explaining that the requested information is absent. Do not treat related "
            "findings, associations, implications, or cited background evidence as "
            "the requested answer.\n\n"

            "When the question attributes a result to the current study, trial, "
            "authors, cohort, or paper, cited background findings from prior work "
...
async def query(request: QueryRequest) -> QueryResponse:
    ...
    system_prompt = (
        ...
        "Do not infer, extrapolate, or invent information that is not explicitly "
        "supported by the context. This is a biomedical question-answering system, "
        "so when the question asks for a specific clinical recommendation, dose, "
        "treatment adjustment, threshold, or other actionable detail, require that "
        "the requested detail be explicitly stated in the context.\n\n"
        
        "Set context_sufficient to true only when the context explicitly contains "
        "enough information to answer the specific question as asked. Related "
        "findings, associations, mechanisms, implications, or suggestions for "
        "future research are not sufficient to answer a question asking for a "
        "specific recommendation or action.\n\n"
        
        "If the context is relevant but does not state the requested information, "
        "say clearly that the information is not stated in the provided context "
        "and set context_sufficient to false. Do not convert an association or "
        "research implication into a clinical recommendation.\n\n"

        "If the question asks what 'this study', 'this trial', 'these authors', "
        "'this cohort', or 'their results' found, only treat evidence as sufficient "
        "when it refers to findings generated by the current paper itself.\n\n"

        "Facts mentioned only as background or cited from prior studies do not "
        "satisfy such a question, even if the exact number or result appears in "
        "the context. In that case, explain the attribution and set "
        "context_sufficient to false.\n\n"
    )
    prompt = f"Context:\n{context}\n\nQuestion: {request.question}"
